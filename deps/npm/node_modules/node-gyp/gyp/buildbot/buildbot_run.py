...
def PrepareAndroidTree():
  ...
  CallSubProcess(
      ['/bin/bash',
       '-c', '{0!s} && make -j4'.format(_ANDROID_SETUP)],
      cwd=ANDROID_DIR)
...
def StartAndroidEmulator():
  ...
  CallSubProcess(['/bin/bash', '-c',
      '{0!s} && adb kill-server '.format(_ANDROID_SETUP)],
      cwd=ANDROID_DIR)
  ...
  CallSubProcess(['/bin/bash', '-c',
      '{0!s} && {1!s} adb start-server '.format(_ANDROID_SETUP, adbd_wrapper)],
      cwd=ANDROID_DIR)

  subprocess.Popen(
      ['/bin/bash', '-c',
       '{0!s} && emulator -no-window'.format(_ANDROID_SETUP)],
      cwd=ANDROID_DIR)
  CallSubProcess(
      ['/bin/bash', '-c',
       '{0!s} && adb wait-for-device'.format(_ANDROID_SETUP)],
      cwd=ANDROID_DIR)
...
def GypTestFormat(title, format=None, msvs_version=None, tests=None):
  ...
  if format == 'android':
    ...
    retcode = subprocess.call(
        ['/bin/bash',
         '-c', '{0!s} && cd {1!s} && {2!s}'.format(_ANDROID_SETUP, ROOT_DIR, command)],
