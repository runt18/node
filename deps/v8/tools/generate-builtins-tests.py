...
def GetVars(result, num, first = []):
  ...
  variables = []
  for i in range(num):
    variables.append("__v_{0:d}".format(g_var_index))
    g_var_index += 1
  for var in variables:
    result.append("var {0!s} = {1!s};".format(var, GetRandomObject()))
  return ", ".join(first + variables)


# Wraps |string| in try..catch.
def TryCatch(result, string, exception_behavior = ""):
  result.append("try {{ {0!s} }} catch(e) {{ {1!s} }}".format(string, exception_behavior))
...
def BuildTests(function, full_name, options):
  ...
  TryCatch(result, "{0!s}({1!s});".format(full_name, GetVars(result, length)))

  if "prototype" in function:
    proto = function["prototype"]
    result.append("\n// Constructor test:")
    TryCatch(result,
             "var recv = new {0!s}({1!s});".format(full_name, GetVars(result, length)),
             "var recv = new Object();")
    ...
    if getters:
      ...
      for getter in getters:
        result.append("print(recv.{0!s});".format(getter["name"]))
    if methods:
      result.append("\n// Method tests:")
      for method in methods:
        args = GetVars(result, method["length"], ["recv"])
        call = "{0!s}.prototype.{1!s}.call({2!s})".format(full_name, method["name"], args)
        TryCatch(result, call)

  filename = os.path.join(options.outdir, "{0!s}.js".format((full_name)))
...
def GenerateTests(options):
  ...
  output = subprocess.check_output(
      "{0!s} {1!s}".format(options.d8, options.script), shell=True).strip()
