...
class _String(_Type):
  ...
  def ValidateMSVS(self, value):
    if not isinstance(value, basestring):
      raise ValueError('expected string; got {0!r}'.format(value))
...
class _StringList(_Type):
  ...
  def ValidateMSVS(self, value):
    if not isinstance(value, basestring) and not isinstance(value, list):
      raise ValueError('expected string list; got {0!r}'.format(value))
...
class _Boolean(_Type):
  ...
  def _Validate(self, value):
    if value != 'true' and value != 'false':
      raise ValueError('expected bool; got {0!r}'.format(value))
...
class _Enumeration(_Type):
  ...
  def ValidateMSBuild(self, value):
    if value not in self._msbuild_values:
      raise ValueError('unrecognized enumerated value {0!s}'.format(value))

  def ConvertToMSBuild(self, value):
    index = int(value)
    if index < 0 or index >= len(self._label_list):
      raise ValueError('index value ({0:d}) not in expected range [0, {1:d})'.format(index, len(self._label_list)))
    label = self._label_list[index]
    if label is None:
      raise ValueError('converted value for {0!s} not specified.'.format(value))
    return label
  ...
  def _Translate(value, msbuild_settings):
    if value == 'true':
      ...
      if 'AdditionalOptions' in tool_settings:
        new_flags = '{0!s} {1!s}'.format(tool_settings['AdditionalOptions'], flag)
      else:
        new_flags = flag
...
def _CustomGeneratePreprocessedFile(tool, msvs_name):
  def _Translate(value, msbuild_settings):
    ...
    if value == '0':
      ...
      raise ValueError('value must be one of [0, 1, 2]; got {0!s}'.format(value))
...
fix_vc_macro_slashes_regex = re.compile(
  r'(\$\((?:{0!s})\))(?:[\\/]+)'.format("|".join(fix_vc_macro_slashes_regex_list))
)
      ...
      for setting, value in settings[tool_name].iteritems():
        if setting in tool_validators:
          try:
            ...
            print >> stderr, ('Warning: for {0!s}/{1!s}, {2!s}'.format(tool_name, setting, e))
        else:
          _ValidateExclusionSetting(setting,
                                    tool_validators,
                                    ('Warning: unrecognized setting {0!s}/{1!s}'.format(tool_name, setting)),
                                    stderr)

    else:
      print >> stderr, ('Warning: unrecognized tool {0!s}'.format(tool_name))
