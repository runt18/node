...
def _ConfigFullName(config_name, config_data):
  ...
  return '{0!s}|{1!s}'.format(_ConfigBaseName(config_name, platform_name), platform_name)
  ...
  if cygwin_shell:
    ...
    if has_input_path:
      direct_cmd = [i.replace('$(InputPath)',
                              '`cygpath -m "${INPUTPATH}"`')
                    for i in direct_cmd]
    direct_cmd = ['\\"{0!s}\\"'.format(i.replace('"', '\\\\\\"')) for i in direct_cmd]
    ...
    if quote_cmd:
      ...
      # TODO(quote):  regularize quoting path names throughout the module
      arguments = ['"{0!s}"'.format(i) for i in arguments]
...
def _GenerateNativeRulesForMSVS(p, rules, output_dir, spec, options):
  ...
  """
  rules_filename = '{0!s}{1!s}.rules'.format(spec['target_name'],
                                   options.suffix)
...
def _GenerateExternalRules(rules, output_dir, spec,
  ...
  """
  filename = '{0!s}_rules{1!s}.mk'.format(spec['target_name'], options.suffix)
  ...
  mk_file.write('all: {0!s}\n'.format(' '.join(first_outputs_cyg)))
  for od in all_output_dirs:
    if od:
      mk_file.write('\tmkdir -p `cygpath -u "{0!s}"`\n'.format(od))
  mk_file.write('\n')
  ...
  for rule in rules:
    ...
    for tf in trigger_files:
      ...
      cmd = ['"{0!s}"'.format(i) for i in cmd]
      cmd = ' '.join(cmd)
      # Add it to the makefile.
      mk_file.write('{0!s}: {1!s}\n'.format(' '.join(outputs), ' '.join(inputs)))
      mk_file.write('\t{0!s}\n\n'.format(cmd))
  # Close up the file.
  mk_file.close()
  ...
  _AddActionStep(actions_to_add,
                 ...
                 description='Running external rules for {0!s}'.format(
                     spec['target_name']),
                 command=cmd)
...
def _GetGuidOfProject(proj_path, spec):
  ...
  if guid:
    if VALID_MSVS_GUID_CHARS.match(guid) is None:
      raise ValueError('Invalid MSVS guid: "{0!s}".  Must match regex: "{1!s}".'.format(guid, VALID_MSVS_GUID_CHARS.pattern))
    guid = '{{{0!s}}}'.format(guid)
...
def _GetMSVSConfigurationType(spec, build_file):
  ...
  try:
    ...
    if spec.get('type'):
      ...
      raise GypError('Missing type field for target {0!s} in {1!s}.'.format(spec['target_name'], build_file))
  return config_type
  ...
  for cpy in spec.get('copies', []):
    for src in cpy.get('files', []):
      ...
      if src.endswith('/'):
        ...
        cmd = 'cd "{0!s}" && xcopy /e /f /y "{1!s}" "{2!s}\\{3!s}\\"'.format(
            _FixPath(base_dir), outer_dir, _FixPath(dst), outer_dir)
        copies.append(([src], ['dummy_copies', dst], cmd,
                       'Copying {0!s} to {1!s}'.format(src, dst)))
      else:
        cmd = 'mkdir "{0!s}" 2>nul & set ERRORLEVEL=0 & copy /Y "{1!s}" "{2!s}"'.format(
            _FixPath(cpy['destination']), _FixPath(src), _FixPath(dst))
        copies.append(([src], [dst], cmd, 'Copying {0!s} to {1!s}'.format(src, dst)))
  return copies
...
def _GetPlatformOverridesOfProject(spec):
  ...
  for config_name, c in spec['configurations'].iteritems():
    ...
    fixed_config_fullname = '{0!s}|{1!s}'.format(
...
def _CreateProjectObjects(target_list, target_dicts, options, msvs_version):
  ...
  for qualified_target in target_list:
    ...
    if spec['toolset'] != 'target':
      raise GypError(
          'Multiple toolsets not supported in msvs build (target {0!s})'.format(
          qualified_target))
  ...
  for source in sources:
    if isinstance(source, MSVSProject.Filter):
      ...
      if not parent_filter_name:
        ...
        filter_name = '{0!s}\\{1!s}'.format(parent_filter_name, source.name)
...
class MSBuildRule(object):
  ...
  def __init__(self, rule, spec):
    ...
    self.compute_output = 'Compute{0!s}Output'.format(self.rule_name)
    ...
    content.extend([
        ...
        ['PropertyGroup',
         [rule.depends_on,
          ...
          '_SelectedFiles;$({0!s})'.format(rule.depends_on)
         ],
        ],
        ['ItemDefinitionGroup',
         [rule.rule_name,
...
def _GenerateMSBuildRuleTargetsFile(targets_path, msbuild_rules):
  ...
  for rule in msbuild_rules:
    ...
    inputs_section = [
        'ItemGroup',
        [rule.inputs, {'Include': '%({0!s}.AdditionalDependencies)'.format(rule_name)}]
    ]
    logging_section = [
        'ItemGroup',
        [rule.tlog,
         {'Include': '%({0!s}.Outputs)'.format(rule_name),
          'Condition': ("'%%(%s.Outputs)' != '' and "
                        "'%%(%s.ExcludedFromBuild)' != 'true'" %
                        (rule_name, rule_name))
         },
         ['Source', "@({0!s}, '|')".format(rule_name)],
         ['Inputs', "@({0!s} -> '%(Fullpath)', ';')".format(rule.inputs)],
        ],
    ]
    message_section = [
        'Message',
        {'Importance': 'High',
         'Text': '%({0!s}.ExecutionDescription)'.format(rule_name)
        }
    ]
    write_tlog_section = [
        'WriteLinesToFile',
        {'Condition': "'@(%s)' != '' and '%%(%s.ExcludedFromBuild)' != "
         ...
         'Lines': "^%({0!s}.Source);@({1!s}->'%(Fullpath)')".format(rule.tlog,
                                                            rule.tlog)
        }
    ]
    read_tlog_section = [
        ...
        {'Condition': "'@(%s)' != '' and '%%(%s.ExcludedFromBuild)' != "
         ...
         'Lines': "^%({0!s}.Source);%({1!s}.Inputs)".format(rule.tlog, rule.tlog)
        }
    ]
    command_and_input_section = [
        ...
        {'Condition': "'@(%s)' != '' and '%%(%s.ExcludedFromBuild)' != "
         ...
         'CommandLineTemplate': '%({0!s}.CommandLineTemplate)'.format(rule_name),
         'AdditionalOptions': '%({0!s}.AdditionalOptions)'.format(rule_name),
         'Inputs': rule_inputs
        }
    ]
    content.extend([
        ['Target',
         {'Name': rule.target_name,
          'BeforeTargets': '$({0!s})'.format(rule.before_targets),
          'AfterTargets': '$({0!s})'.format(rule.after_targets),
          'Condition': "'@({0!s})' != ''".format(rule_name),
          'DependsOnTargets': '$({0!s});{1!s}'.format(rule.depends_on,
                                            rule.compute_output),
        ...
        ['PropertyGroup',
         ['ComputeLinkInputsTargets',
          '$(ComputeLinkInputsTargets);',
          '{0!s};'.format(rule.compute_output)
         ],
        ...
        ['Target',
         ...
         ['ItemGroup',
          ...
          ['Link',
           {'Include': '%({0!s}.Identity)'.format(rule.dirs_to_make),
         ...
         ['MakeDir',
          {'Directories': ("@({0!s}->'%(RootDir)%(Directory)')".format(
                           rule.dirs_to_make))
          }
         ]
        ],
        ...
        ['Rule',
         ...
         ['DynamicEnumProperty',
          ...
          ['DynamicEnumProperty.ProviderSettings',
           ['NameValuePair',
            {'Name': 'Exclude',
             'Value': '^{0!s}|^Compute'.format(rule.before_targets)
            }
           ]
          ],
         ...
         ['DynamicEnumProperty',
          ...
          ['DynamicEnumProperty.ProviderSettings',
           ['NameValuePair',
            {'Name': 'Exclude',
             'Value': '^{0!s}|^Compute'.format(rule.after_targets)
            }
           ]
          ],
          ['DynamicEnumProperty.DataSource',
...
def _GetConfigurationCondition(name, settings):
  return (r"'$(Configuration)|$(Platform)'=='{0!s}|{1!s}'".format(*
...
def _GetMSBuildProjectConfigurations(configurations):
  ...
  for (name, settings) in sorted(configurations.iteritems()):
    ...
    designation = '{0!s}|{1!s}'.format(configuration, platform)
    group.append(
  ...
  if not props_specified:
    return [
        ['ImportGroup',
         ...
         ['Import',
          {'Project': user_props,
           'Condition': "exists('{0!s}')".format(user_props),
...
def _GetMSBuildConfigurationGlobalProperties(spec, configurations, build_file):
  ...
  if cygwin_dirs:
    cyg_path = '$(MSBuildProjectDirectory)\\{0!s}\\bin\\'.format(_FixPath(cygwin_dirs))
